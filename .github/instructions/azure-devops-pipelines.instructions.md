...
Each feature in the Samples repo should have its own CI pipeline. Create a new pipeline file named `SamplesCI-<FeatureName>.yml` at the repository root:

```yaml
# SamplesCI-<FeatureName>.yml
...
3. **Path Filters**: Include `Samples/<FeatureName>`
...
Ensure the full `SamplesCI-<FeatureName>` check is added, not individual job configurations.
