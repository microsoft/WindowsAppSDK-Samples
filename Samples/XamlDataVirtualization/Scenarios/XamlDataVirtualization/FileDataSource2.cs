...
namespace DataVirtualizationSample
{
    ...
    class FileDataSource2 : INotifyCollectionChanged, System.Collections.IList, IItemsRangeInfo, ISelectionInfo
    {
        ...
        private FileDataSource2()
        {
            ...
            _dispatcher = SDKTemplate.App.MainDispatcherQueue;
        ...
        private void QueryResult_ContentsChanged(IStorageQueryResultBase sender, object args)
        {
            ...
            {
                _dispatcher.TryEnqueue(ResetCollection);
            }
            else
            {
                ResetCollection();
        ...
        // A shared, non-null placeholder returned while an item is still being fetched.
        // UWP tolerated returning null from a virtualized IList; WinUI 3's ListViewBase
        // fail-fasts (combase E_POINTER) on null items, so we hand back an empty FileItem
        // and let the CacheChanged -> Replace notification swap in the real item once loaded.
        private static readonly FileItem PlaceholderItem = new FileItem();

        public object this[int index]
        {
            get
            {
                // The cache returns null if it doesn't have the item yet. Once the item is
                // fetched it fires a Replace notification so the list control updates.
                return itemCache[index] ?? PlaceholderItem;
            }
            set
            {
