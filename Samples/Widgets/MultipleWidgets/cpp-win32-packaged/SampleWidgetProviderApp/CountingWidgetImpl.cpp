...
CountingWidget::CountingWidget(winrt::hstring const &id, winrt::hstring const& state) : m_clickCount{ 0 }, WidgetImpl(id, state)
{
	if (state.empty())
	{
		State(L"0");
...
void CountingWidget::Activate()
{
	...
	// except when 'Incremen' button was clicked - 
