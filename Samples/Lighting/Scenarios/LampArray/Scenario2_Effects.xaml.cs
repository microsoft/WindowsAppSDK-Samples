...
namespace SDKTemplate
{
    ...
    public sealed partial class Scenario2_Effects : Page
    {
        ...
        private async void StaticBitmapButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            Uri bitmapUri = new Uri("ms-appx:///Assets/StoreLogo.png");
        ...
        // This effect creates a simple animation. On each frame, we
        ...
        private void GeneratedBitmapButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            foreach (LampArrayInfo info in m_attachedLampArrays)
            {
                ...
                state.squarePixels = new byte[state.bitmapWidth * state.bitmapHeight * 4];
        ...
        private static void GeneratedBitmapEffect_UpdateRequested(GeneratedBitmapState state, LampArrayBitmapRequestedEventArgs args)
        {
            ...
            float squareEndX = squareX + state.squareLength;
            float squareEndY = state.squareY + state.squareLength;

            for (int y = 0; y < state.bitmapHeight; y++)
            {
                for (int x = 0; x < state.bitmapWidth; x++)
                {
                    Color color = x >= squareX && x < squareEndX && y >= state.squareY && y < squareEndY ? Colors.Red : Colors.Blue;
                    int offset = ((y * state.bitmapWidth) + x) * 4;
                    state.squarePixels[offset] = color.B;
                    state.squarePixels[offset + 1] = color.G;
                    state.squarePixels[offset + 2] = color.R;
                    state.squarePixels[offset + 3] = color.A;
                }
            }
            ...
            {
                writer.WriteBytes(state.squarePixels);
                state.squareBitmap.CopyFromBuffer(writer.DetachBuffer());
            }
