    ...
    public sealed partial class Scenario4_Connect : Page
    {
        ...
        private async Task UpdateConnectivityStatusAsync()
        {
            ...
            if (connectedProfile != null && !connectedProfile.ProfileName.Equals(savedProfileName))
            {
                ...
                NotifyUserOnUiThread(string.Format("WiFi adapter connected to: {0} ({1})", connectedProfile.ProfileName, connectedProfile.GetNetworkConnectivityLevel()), NotifyType.StatusMessage);
            }
            else if (connectedProfile == null && savedProfileName != null)
            {
                savedProfileName = null;
                NotifyUserOnUiThread("WiFi adapter disconnected", NotifyType.StatusMessage);
            }
        }

        private void NotifyUserOnUiThread(string message, NotifyType notifyType)
        {
            ...
            {
                App.MainDispatcherQueue.TryEnqueue(() => rootPage.NotifyUser(message, notifyType));
            }
        }
