...
{
    /// <summary>
    /// A page that demonstrates adaptive x:DeferLoadStrategy with the Visual State Manager.
    /// </summary>
    public sealed partial class AdaptivePage : Page
    {
        ...
        public AdaptivePage()
        {
            ...
            // UWP used ApplicationView.GetForCurrentView().VisibleBoundsChanged, which is not available on
            // WinUI 3 desktop. The Page's SizeChanged event provides the equivalent reactive width.
            this.SizeChanged += AdaptivePage_SizeChanged;
        }

        private void AdaptivePage_SizeChanged(object sender, SizeChangedEventArgs e)
        {
            //Here we are hiding panes when we get to certain thresholds
            if (e.NewSize.Width < 768 && ReadingPane != null)
                ReadingPane.Visibility = Visibility.Collapsed;

            if (e.NewSize.Width < 1024 && AccountsList != null)
