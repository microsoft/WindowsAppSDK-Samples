...
namespace SDKTemplate.ViewModels
{
    public class SecureStopViewModel : ViewModelBase
    {
        ...
        // The ContentHeader for the actively playing asset is tracked.
        public PlayReadyContentHeader activePlayReadyHeader { get; private set; }

        /// <summary>
        /// A view model class for the Secure Stop Scenario.
        /// </summary>
        public SecureStopViewModel(MediaPlayerElement mediaElement)
        {
            ...
            licenseSession.ConfigureMediaProtectionManager(player.ProtectionManager);

            PlayReadyInfo = new PlayReadyInfoViewModel();
            PlayReadyInfo.RefreshStatics();

            CmdPlayMovie = new RelayCommand(
                                    () => { player.Source = MediaSource.CreateFromUri(new Uri(moviePath)); SetPlaybackEnabled(true); },
                                    () => publisherCert != null);

            // UWP fired the secure-stop logic on MediaElementState.Closed (a hard teardown).
            // MediaPlayer has no equivalent "Closed" state, so the stop logic runs explicitly
            // when the user stops playback.
            CmdStopMovie = new RelayCommand(() => { StopPlayback(); },
                                            () => publisherCert != null);
            ...
            // Natural end of playback is also a stop: send the secure-stop records.
        ...
        /// <summary>
        /// Stops playback and sends secure-stop notifications. In UWP this ran when the MediaElement
        /// transitioned to the Closed state; here it is invoked explicitly on stop / end of media.
        /// </summary>
        ...
        private async void GetPublisherCert(string publisherID, Action<byte[]> callback)
        {
            ...
            if (!localSettings.Containers["PublisherCerts"].Values.ContainsKey(publisherID))
            {
                ...
                var content = new System.Net.Http.StringContent(xDoc.ToString(), System.Text.Encoding.UTF8, "text/xml");
