...
namespace SDKTemplate
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        // When navigation is triggered by an activation (a file/protocol arriving), we set
        // NavView.SelectedItem programmatically. That would raise NavView_SelectionChanged, which
        // navigates WITHOUT the activation payload and would clobber it. This guard suppresses that.
        ...
        private static NavigationViewItem FindItemForType(System.Collections.Generic.IList<object> items, Type scenarioType)
        {
            foreach (object o in items)
            {
                if (o is NavigationViewItem item)
                {
                    if (item.Tag is Type t && t == scenarioType)
                    {
                        return item;
                    }

                    NavigationViewItem child = FindItemForType(item.MenuItems, scenarioType);
                    if (child != null)
                    {
                        return child;
                    }
        ...
        /// <summary>
        /// Navigates to a scenario page and passes <paramref name="parameter"/> to it. Used by the
        /// activation pipeline (App.RouteActivation) so the "Receiving" scenarios can read the
        /// incoming file/protocol payload from their OnNavigatedTo. The matching menu item is
        /// selected without re-triggering parameterless navigation.
        /// </summary>
        public void NavigateToScenario(Type scenarioType, object parameter)
        {
            NotifyUser(string.Empty, NotifyType.StatusMessage);

            ScenarioFrame.Navigate(scenarioType, parameter);

            NavigationViewItem item = FindItemForType(NavView.MenuItems, scenarioType);
