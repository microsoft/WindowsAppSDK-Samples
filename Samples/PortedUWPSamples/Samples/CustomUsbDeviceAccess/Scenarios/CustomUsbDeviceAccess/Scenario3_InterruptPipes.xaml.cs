...
namespace SDKTemplate
{
    ...
    public sealed partial class Scenario3_InterruptPipes : Page
    {
        ...
        private void RegisterOsrFx2InterruptEvent_Click(Object sender, Microsoft.UI.Xaml.RoutedEventArgs eventArgs)
        {
            if (EventHandlerForDevice.Current.IsDeviceConnected)
            {
        ...
        /// <param name="sender"></param>
        /// <param name="eventArgs"></param>
        private void RegisterSuperMuttInterruptEvent_Click(Object sender, Microsoft.UI.Xaml.RoutedEventArgs eventArgs)
        {
            if (EventHandlerForDevice.Current.IsDeviceConnected)
            {
        ...
        private void UnregisterInterruptEvent_Click(Object sender, Microsoft.UI.Xaml.RoutedEventArgs eventArgs)
        {
            if (EventHandlerForDevice.Current.IsDeviceConnected)
            {
                UnregisterFromInterruptEvent();
        ...
        /// <param name="sender"></param>
        /// <param name="eventArgs"></param>
        private void WriteSuperMuttInterruptOut_Click(Object sender, Microsoft.UI.Xaml.RoutedEventArgs eventArgs)
        {
            if (EventHandlerForDevice.Current.IsDeviceConnected)
            {
        ...
        private async void OnOsrFx2SwitchStateChangeEvent(UsbInterruptInPipe sender, UsbInterruptInEventArgs eventArgs)
        {
            ...
            if (!navigatedAway)
            {
                ...
                if (buffer.Length > 0)
                {
                    ...
                    // Use the dispatcher queue because we are printing the table to the UI directly and the
        ...
        private async void OnGeneralInterruptEvent(UsbInterruptInPipe sender, UsbInterruptInEventArgs eventArgs)
        {
            ...
            if (!navigatedAway)
            {
                ...
                // Use the dispatcher queue because we are interracting with the UI directly and the
                // thread that this function is running on may not be the UI thread; if a non-UI thread modifies
                // the UI, an exception is thrown
                App.MainDispatcherQueue.TryEnqueue(() =>
            {
