...
{
    // Bootstrap initialization is handled automatically by the WinAppSDK build targets
    // via MddBootstrapAutoInitializer.cpp linked through Foundation_Framework target
    try
    {
        ...
        // Use dynamic loading for framework-dependent (DLL is in the Framework MSIX package)
        ...
        if (dwriteModule)
        {
            ...
            if (createFactory)
            {
                ...
                message << L"DWriteCore: " << (SUCCEEDED(hr) ? L"Loaded" : L"CreateFactory failed") << L"\n";
            }
            else
            {
