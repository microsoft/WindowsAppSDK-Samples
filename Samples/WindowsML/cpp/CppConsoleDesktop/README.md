...
Options:
  --ep_policy <policy>
      Set execution provider policy (NPU, CPU, GPU, DEFAULT). Default: DEFAULT
  ...
  --compiled_output <path>
      Path for compiled output model (default: auto-generated with device info)
...
The sample shows how to compile an ONNX model for optimized execution. Compiled
model filenames are generated with device-specific identifiers to prevent
collisions:
