...
accepted. Missing metadata and every other compatibility result cause the WPF
sample to use the original ONNX model.

The WPF UI does not request compilation, so it does not replace a missing or
non-optimal cache. Applications that enable the shared `Options.Compile` path
